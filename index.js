...
const AUTHORIZED_ROLE_ID = "1054506177406451793";
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.customId === "select_time_slot") {
    ...
    await interaction.channel.send(
      `✅ ${interaction.user}, vous avez choisi la tranche horaire : **${chosenLabel}**. Merci de votre engagement !`,
    );
    ...
    if (newChannelName && interaction.channel && interaction.channel.edit) {
      try {
        ...
        console.log(
          `Salon renommé en "${newChannelName}" suite au choix de ${interaction.user.tag}`,
        );
      } catch (error) {
